...
module.exports.run = async(client, message, args)  => {
 ...
 if(!message.member.roles.cache.get(ayarlar.kickyetkili) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send('Bu Komutu Kullanmak İçin Yetkiniz Bulunmamakta.')
...
if(member.hasPermission("ADMINISTRATOR")) return message.channel.send('Yetkilileri Banlayamam!')
  member.kick({days: 7, reason: reason})
        ...
        .setColor(`GREEN`)
        .setDescription(`<@${member.id}> Adlı Kullanıcı <@!${message.author.id}> Yetkilisi Tarafından **${reason}** Sebebiyle Kicklendi!`)
        message.channel.send(embed)
  
  const log = new Discord.MessageEmbed()
  .setTitle(`Bir Kullanıcı Kick!`)
  .setDescription(`**Kicklenen:** <@!${member.id}> 
